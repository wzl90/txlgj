...
private:
    ...
    static const uint16_t sm_yearDay[YEAR_NUM];         // 每年的天数
//    static const uint8_t sm_accDay[YEAR_NUM];   //每年公历1月1日到农历一月一日相差的天数
...
public:
    ...
    static int GetYearDays(int year)
    {
        ...
        return sm_yearDay[year-YEAR_BASE];
    }
//    static int GetYearDays(int year)
//    {
...
//    }
    /*
     * 查询公历1月1日到农历一月一日相差的天数
     * @param year: 要查询的年份，有效取值范围1901~2050
     * @return: 返回该年公历1月1日到农历一月一日相差的天数
     *          如果参数year不在有效值范围内，则抛出异常ChineseCalendarDB::InvalidParamter
     */
//    static int GetAccDays(int year)
//    {
//        if (year < YEAR_BASE || year >= YEAR_BASE + YEAR_NUM)
//            throw InvalidParamter(); // 没有该年的数据
//        
//        return sm_accDay[year-YEAR_BASE];
//    }
