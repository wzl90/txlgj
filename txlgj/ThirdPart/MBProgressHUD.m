...
@synthesize interceptRecognizer;
...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
        ...
        //拦截navigationController的手势返回
        self.interceptRecognizer = [[UIGestureRecognizer alloc]init];
        [self addGestureRecognizer:self.interceptRecognizer];
	}
	return self;
}
...
- (void)hideUsingAnimation:(BOOL)animated {
	// Fade out
//	if (animated && showStarted)自己修改源文件了，hud隐藏的时候animated为no，self.showStarted = nil在ios6、7上因为野指针闪退了。
    if (animated)
    {
		[UIView beginAnimations:nil context:NULL];
		[UIView setAnimationDuration:0.30];
